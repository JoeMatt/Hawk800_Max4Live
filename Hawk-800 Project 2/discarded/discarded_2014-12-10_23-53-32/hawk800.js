	...
	for (var i=0;i<length;i++){
		...
		if (maxObject !== undefined && maxObject !== null) {
			...
			if (min !== undefined) {
				...
				var range = max - min;
				var multiplier = 127.0 / range;
				rect = lastBox.rect;
				var newMult = this.patcher.newdefault(rect[0], rect[3] + 8 + 1500, "*", multiplier);
				this.patcher.hiddenconnect(lastBox, 0, newMult, 0);
				GenObjects.push(newMult);
				lastBox = newMult;

				// if (min > 0) {
				// 	rect = lastBox.rect;
				// 	var newAdd = this.patcher.newdefault(rect[0], rect[3] + 8 + 1500, "+", min);
				// 	this.patcher.hiddenconnect(lastBox, 0, newAdd, 0);
				// 	GenObjects.push(newAdd);
				// 	lastBox = newAdd;
				// }
			};
			...
			rect = lastBox.rect;
			var newMsg = this.patcher.newdefault(rect[0], rect[3] + 8, "message");
			 	// Set text
			newMsg.message("set", "NRPN", "$1", global16, cc, 1);
			GenObjects.push(newMsg);
			post("Length "+GenObjects.length+"\n");
				// Connect
			this.patcher.hiddenconnect(lastBox, 0, newMsg, 0);
			lastBox = newMsg;
