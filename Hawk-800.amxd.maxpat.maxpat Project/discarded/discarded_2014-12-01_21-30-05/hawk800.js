	...
	for (var i=0;i<length;i++){
		...
		if (maxObject !== undefined && maxObject !== null) {
			...
			if (cc_value === undefined) {
				...
				post(rect);
				var msgText = "cc $1 -1 "+ cc + " 1";
				var newMsg = this.patcher.newdefault(rect[0], rect[1] + rect[3] + 5, "message");
				newMsg.message("set", msgText);
		
				// connect or hiddenconnect
				this.patcher.connect(maxObject, 0, newMsg, 0); 

				// connect to send
				var send = $("s");
				this.patcher.connect(newMsg, 0, send, 0);
			} else {
